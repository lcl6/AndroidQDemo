package com.example.androidqdemo.theme;
...
public class ThemeManager {
    ...
    public static int getCurrentThemeRes(Context context, int dayResId) {
        ...
        HashMap cachedRes = (HashMap) sCachedNightResrouces.get(typeName);
        // 先从缓存中去取，如果有直接返回该id
        if (cachedRes == null) {
            cachedRes = new HashMap<>();
        }
        Integer resId = (Integer) cachedRes.get(entryName + RESOURCE_SUFFIX);
